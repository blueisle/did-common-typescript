...
/** Options for constructing an HttpUniversalResolver. */
export interface HttpUniversalResolverOptions {
 ...
 */
export default class HttpUniversalResolver implements IDidResolver {
  ...
  private fetchImplementation: GlobalFetch['fetch'];
    ...
    *
    * @param urlOrOptions The endpoint to query (as a string), or else an instance of `HttpUniversalResolverOptions`.
    */
  constructor (urlOrOptions: string | HttpUniversalResolverOptions) {
  ...
  private getDefaultFetchImplementation () {
    ...
    throw new Error('Please pass an implementation of fetch() to the HttpUniversalResolver.');
  }
  ...
  public async resolve (did: string): Promise<IDidResolveResult> {
    ...
    const resDocument = await response.json();

    return {
      didDocument: new DidDocument(resDocument.didDocument),
      metadata: resDocument.resolverMetadata
